...
class Band:
	...
	def __init__(self, bandName: tuple[str, str], startRange: float, freqStep: float, channelsAmount: int, freqFormat: str, bandDivider: int = None):
		self.name, self.tip = bandName
		self.startRange, self.freqStep, self.channelsAmount, self.freqFormat, self.bandDivider = (startRange, freqStep, channelsAmount, freqFormat, bandDivider)
	...
	def __getitem__(self, channel: int) -> FrequencyObject:
		...
		return FrequencyObject(name=f"{bandName} {channel + 1}", frequency=self.freqFormat % result, frequencyRaw=result)

	__len__ = lambda self: self.channelsAmount
...
class ChanToFreqTab(wx.Panel):
	...
	def __init__(self, parent):
		...
		self.bandsChooser = wx.RadioBox(bandsChooserPanel, wx.ID_ANY, "Диапазон:", choices=[band.name + f" ({band.tip})" for band in bandsList], style=wx.RA_SPECIFY_ROWS)
		bandsChooserSizer.Add(self.bandsChooser, wx.EXPAND)
		bandsChooserPanel.SetSizer(bandsChooserSizer)
		self.bandsChooser.SetToolTip("Выберите диапазон, частоту которого вы хотите увидеть. Поддерживаются диапазоны {}.".format(", ".join(([band.tip for band in bandsList]))))
		...
		self.channelsCombo = wx.ComboBox(channelChooser, wx.ID_ANY, choices=[channel.name for channel in bandsList[self.bandsChooser.GetSelection()]], style=wx.CB_READONLY)
		...
		self.frequencyResult = wx.TextCtrl(resultArea, wx.ID_ANY, bandsList[self.bandsChooser.GetSelection()][self.channelsCombo.GetSelection()].frequency + " МГц", style=wx.TE_READONLY | wx.TE_MULTILINE | wx.TE_DONTWRAP)
		freqSizer.Add(self.frequencyResult, wx.EXPAND)
		actionsArea = wx.Panel(resultArea)
		actBtnsSizer = wx.BoxSizer(wx.HORIZONTAL)
		self.copyFrequencyButton = wx.Button(actionsArea, wx.ID_ANY, "Копировать частоту")
		self.copyFrequencyButton.Bind(wx.EVT_BUTTON, self.onActionCopy)
		actBtnsSizer.Add(self.copyFrequencyButton, wx.EXPAND)
		self.copyBFFrequencyButton = wx.Button(actionsArea, wx.ID_ANY, "Копировать частоту для BF480")
		self.copyBFFrequencyButton.Bind(wx.EVT_BUTTON, self.onActionBFCopy)
		self.copyBFFrequencyButton.SetToolTip("Программа BF480 для прошивки радиостанций Baofeng принимает частоты, разделенные не точкой, как это ожидалось бы, а запятой.")
	...
	def onBandSelected(self, _):
		...
		self.channelsCombo.AppendItems([channel.name for channel in bandsList[self.bandsChooser.GetSelection()]])
		self.channelsCombo.SetSelection(0)
	...
	def onActionCopy(self, _):
		if wx.TheClipboard.Open():
			self.clipboard.SetData(wx.TextDataObject(bandsList[self.bandsChooser.GetSelection()][self.channelsCombo.GetSelection()].frequency))
			wx.TheClipboard.Close()

	def onActionBFCopy(self, _):
		if wx.TheClipboard.Open():
			self.clipboard.SetData(wx.TextDataObject(re.sub("[.]", ",", bandsList[self.bandsChooser.GetSelection()][self.channelsCombo.GetSelection()].frequency)))
			wx.TheClipboard.Close()
...
class FreqToChanTab(wx.Panel):
	...
	def __init__(self, parent):
		...
		self.freqField.SetToolTip("Введите здесь частоту одного из поддерживаемых диапазонов. Поддерживается любой формат разделения десятичных частот.")
		...
		self.resultField = wx.TextCtrl(resultArea, wx.ID_ANY, "", style=wx.TE_READONLY|wx.TE_MULTILINE|wx.TE_DONTWRAP)
		self.resultField.AppendText("Введите частоту слева, чтобы узнать ее диапазон и канал. Поддерживаются диапазоны ")
		self.resultField.AppendText(re.sub(r"(.+), (.+)$", r"\1 и \2.", ", ".join([band.tip + f" ({band.name})" for band in bandsList])))
	...
	def onFrequencyProvided(self, event):
		...
		for band in bandsList:
			if (band.startRange - band.freqStep) <= freqProvided <= band[len(band) + 1].frequencyRaw:
				preChannel = ((freqProvided-decimal.Decimal(band.startRange)) / decimal.Decimal(band.freqStep)) + 1
				...
				if int(preChannel) == preChannel:
					...
					self.resultField.ChangeValue(f"Ближе к {band[round(preChannel) - 1].name}, частота которого {band[round(preChannel) - 1].frequency} МГц")
				return True
...
class AppWindow(wx.Frame):
	...
	def __init__(self, *args, **kw):
		super().__init__(None, title="Калькулятор частот", pos=(wx.Center, wx.Center), size=(640, 480))
